...
class ForeignKeyGraph:
    ...
    def __init__(self, tables_schema: dict[str, dict]) -> None:
        """
        Initialize the foreign key graph.

        Args:
            tables_schema (Dict[str, Dict[str, Any]]): Dictionary containing the schema of the tables.
        """
        self.tables = list(tables_schema.keys())
        self.table_to_index = {name: i for i, name in enumerate(self.tables)}
        self.graph: list[type[ForeignKeyGraph.Edge]] = [[] for _ in self.tables]

        self.populate_graph(copy.deepcopy(tables_schema))

    def populate_graph(self, tables_schema: dict[str, dict] = None) -> None:
        """
    ...
    def get_edges(self, table: str) -> list[type["ForeignKeyGraph.Edge"]]:
        """
