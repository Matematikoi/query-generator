...
from udao_trace.utils.benchmarks_schema import (
    get_tpcds_table_info,
    get_tpch_table_info,
)
from udao_trace.utils.foreign_key_graph import ForeignKeyGraph
...
class SubGraphGenerator:
    def __init__(
        self, tables_schema: dict[str, dict], keep_edge_prob: float = 0.5, max_hops=2
    ):
        self.hops = max_hops
    ...
    def mark_subgraph_visited(self, subgraph: int) -> bool:
        """
        Mark a subgraph as visited.
        Args:
            path (int): Hash of subgraph to be marked.
        Returns:
            bool: True if the path was not previously marked, False otherwise.
        """
        if subgraph in self.table_combinations_seen:
            return False
        else:
            self.table_combinations_seen[subgraph] = True
            return True

    def is_subgraph_visited(self, subgraph: int) -> bool:
        """
        Check if a subgraph has been visited.
        Args:
            path (int): Hash of subgraph to be checked.
        Returns:
            bool: True if the path is fully explored, False otherwise.
        """
        return subgraph in self.table_combinations_seen
    ...
    def get_unseen_random_subgraph(self, fact_table: str) -> list[ForeignKeyGraph.Edge]:
        """
        ...
        while True:
            cnt += 1
            if cnt > 1000:
                raise GraphExploredError("Unable to find a new subgraph after 1000 attempts.")
...
class PredicateGenerator:
    ...
    def get_random_predicates(
        self, tables: str, num_predicates: int, row_retention_probability=0.2
    ) -> list["PredicateGenerator.Predicate"]:
        """
        ...
        """
        selected_tables_histogram = self.histogram[self.histogram["table"].isin(tables)]
    ...
    def _get_min_max_from_bins(
        self, bins: str, row_retention_probability
        ...
        """
        bins = eval(bins)
        subrange_length = max(1, round(row_retention_probability / 100 * len(bins)))
        start_index = random.randint(0, len(bins) - subrange_length)

        min_value = bins[start_index]
        max_value = bins[start_index + subrange_length - 1]
        return min_value, max_value


class QueryBuilder:
    def __init__(
        self, tables_schema: dict[str, dict], benchmark: BenchmarkType, **kwargs
    ):
...
class QueryWriter:
    ...
    def write_query(self, query: str, file_name: str):
        """
...
def generate_and_write_queries(
    schema_function: Callable[[], tuple[dict[str, dict], list[str]]],
    max_hops: int,
