...
database. This occurs on the `generate-db` endpoint.
The file used to generate data is the 
[setup file](../../src/query_generator/duckdb_connection/setup.py).


## Calculating the stats 
To calculate the stats we use for generating synthetic data we use a duckdb 
database that we query to get the stats we need. Stats are calculated
on the `make-histograms` endpoint, specifically on the 
[utils file](../../src/query_generator/duckdb_connection/utils.py)
of the duckdb connection module.
The [histogram file](../../src/query_generator/tools/histograms.py)
works interpreting the data from duckdb and it parses it into parquet.

## Filtering the synthetic data
The data that is generated by the synthetic generator is validated in 
the [synthetic query generator file](../../src/query_generator/synthetic_queries/synthetic_query_generator.py) 
using duckdb. Particularly on the `get_result_from_duckdb` function.

We use duckdb to confirm that the synthetic query is indeed valid,
and to get the cardinality of the query. The cardinality of the query
is used to filter queries, by our empty-set filter and our stratified
sampling.
...
a small dataset. Particularly we do it on the `extensions-with-ollama`
endpoint, in the (query validation file)[../../src/query_generator/duckdb_connection/query_validation.py]. Particularly on the 
`is_query_valid` function.

## Trace collection - Fix transform
We use duckdb to collect traces after we do LLM augmentation.
We also use DuckDB to get the schema to make a fix using SQLglot.

On the [fix transform file](../../src/query_generator/extensions/fix_transform.py)
we use the `get_duckdb_schema` to get the schema from duckdb. We
also use it to get the cardinality in the 
[query validation file](../../src/query_generator/duckdb_connection/query_validation.py) 
on the `get_query_output_size` function.

Finally we collect the trace on the 
[fix transform file](../../src/query_generator/extensions/fix_transform.py)
, specifically on the `get_trace_from_transform` function.
